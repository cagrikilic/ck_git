...
r=0.105;%radius (m)
b=0.090;%width(m)
k=0.100;%shear deformation modulus
sampling=0.05;%20hz
j1=0;
for jj=1:100
for j=1:0.05:30

V=0.565+0.001*rand;%m/sec Linear velocity
if V<0
    V=V*(-1);
end
w=V/r+0.001*rand;%angular velocity
if w<0
    w=w*(-1);
end

theta1=(20+0.1*rand)*pi/180;%the angle from the vertical at which the wheel first makes contact
if theta1<0
    theta1=theta1*(-1);
end
W=67+0.01*rand; %kg vertical load
if W<0
    W=W*(-1);
end

T=0.045+0.001*rand;% Torque= (moment of inertia)(angular acceleration)
